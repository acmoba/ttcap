...
var fonts = ["SimHei", "SimSun", "NSimSun", "FangSong", "Microsoft YaHei", "Microsoft JhengHei", "KaiTi", "PMingLiU", "MingLiU"];
if (!iswindows) {
    var Font = canvas.Font;
    var FZSTKFont = new Font('FZSTK', fontFile('FZSTK.TTF'));
    var SIMYOUFont = new Font('SIMYOU', fontFile('SIMYOU.TTF'));
    var STXINGKAFont = new Font('STXINGKA', fontFile('STXINGKA.TTF'));
    var cus_fonts = [FZSTKFont, SIMYOUFont, STXINGKAFont];
    fonts = ['FZSTK', 'msyh', 'SIMYOU', 'STXINGKA'];
}
...
function drawCaptcha(captchaObj) {
    ...
    for (var i = 0; i < word_question.length; i++) {
        ...
        if (iswindows) _ctx.font = randArray([25, 26, 27, 28]).toString() + "px " + randArray(fonts);
            ...
            _ctx.addFont(randArray(cus_fonts[_tt_rdm]));
